...
impl MacosDisplay {
    ...
    unsafe fn update_dimensions(&mut self) -> Option<(i32, i32)> {
        ...
        if d.high_dpi {
            // [KuberNation patch 1/2 — upstream 14c6fc31] backingScaleFactor
            // returns 0.0 while the window is not attached to a screen (startup,
            // and transiently during display/zoom transitions on multi-monitor
            // setups). Recording dpi_scale = 0 propagates 0x0/NaN dimensions
            // downstream; keep the previous (valid) scale until a real value
            // lands. Backported from miniquad master (unreleased; macroquad
            // pins =0.4.10, hence this vendored copy).
...
pub fn define_cocoa_window_delegate() -> *const Class {
    ...
    extern "C" fn window_did_resize(this: &Object, _: Sel, _: ObjcId) {
        let payload = get_window_payload(this);
        // [KuberNation patch 2/2 — companion] Defer the drawable/dimension
        // refresh while the user is interactively resizing, matching the GL
        // draw_rect skip: update_dimensions runs `[gl_context update]` (a
        // drawable reallocation) on every drag step, the same GL-mid-resize
        // churn the patch exists to avoid. The first main-loop frame after
        // the drag ends refreshes dimensions unconditionally.
        if !payload.view.is_null() {
            let live: BOOL = unsafe { msg_send![payload.view, inLiveResize] };
            if live != NO {
                return;
            }
        }
...
pub fn define_opengl_view_class() -> *const Class {
    ...
    extern "C" fn draw_rect(this: &Object, _sel: Sel, _: ObjcId) {
        ...
        let payload = get_window_payload(this);
        unsafe {
            // [KuberNation patch 2/2] Do NOT touch GL while the user is
            // interactively resizing the window. Apple's deprecated GL-on-Metal
            // layer (AppleMetalOpenGLRenderer) can EXC_BAD_ACCESS when an app
            // draws mid-live-resize on macOS 26 + Apple Silicon (see
            // liballeg/allegro5#1749 — a guaranteed crash there); this
            // draw_rect runs a FULL app frame from the resize tracking loop,
            // which is exactly that trigger. The skip sits BEFORE
            // update_dimensions so `[gl_context update]` (a drawable
            // reallocation — GL churn of the same class) is deferred too: the
            // last-presented surface stays intact and the compositor stretches
            // it for the duration of the drag, rather than displaying a
            // resized-but-never-presented drawable (implementation-defined —
            // could be garbage). Dimensions + content refresh on the first
            // main-loop frame after the tracking loop ends (perform_redraw →
            // update_dimensions is unconditional there). Scoped to
            // inLiveResize, so the fullscreen/zoom animations (which survive
            // fine) keep painting.
            // (`!= NO`, not `== YES` — objc-rs's BOOL is c_schar on x86_64
            // where any non-zero is truthy; the crate's own doc says to
            // compare with NO. `== YES` would fail-open on a truthy non-1.)
