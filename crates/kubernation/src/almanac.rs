...
fn page_controls(cx: &mut Ctx) {
    ...
    cx.para(
        "The Wonders menu holds the Oracle: a bring-your-own language model you consult to EXPLAIN a scope — the whole realm, a selected workload or node, or a focused concern — and which may PROPOSE a fix (scale / restart / set-image / rollback / cordon). It is advisory: the model sees a redacted, fenced summary of what Kubernation already observed (never raw cluster dumps, never Secret values) and can NEVER change the cluster itself — a proposal is validated against the live cluster and offered as a Stage button that enters the planning turn, applied only through the usual confirmed, RBAC-checked, dry-run gate. Use Preview to review exactly what will be sent before you Consult. Configure endpoints in the Oracle window's Settings face: save named PROFILES (a local Ollama, a corporate frontier model, …) and switch between them. For a local endpoint it lists the models you've pulled (click to pick); for a remote one you enter a URL + a masked API token + model. Profiles persist to ~/.config/kubernation/oracle.json, including the token by your explicit per-profile opt-in (PLAINTEXT on disk at mode 0600 — for high-sensitivity tokens prefer the KUBERNATION_LLM_TOKEN env var, which is never persisted). A remote endpoint publishes off-laptop, so it stays off until you Arm it (switching profiles re-disarms). The default is a local Ollama at localhost:11434 with model qwen3.5:35b (must be pulled); --llm-url / --llm-model override the active profile for one launch. A remote endpoint publishes off-laptop, so it is off until you Arm it; replies are model-generated — verify before acting.",
    );
}
