...
fn page_world(cx: &mut Ctx) {
    ...
    cx.para(
        "Nodes also belong to a NODEPOOL, and a pool spanning three zones is normal. Zone stays primary - it is the failure domain, and it is what the columns are - so such a pool reads as three regions rather than one shape, which is the truth a zone outage makes matter. Pool identity travels by COLOUR and LABEL rather than by position: View \u{25b8} Pool tints each province by its pool, names each region on the map, and lists every pool in the right column with how many nodes and zones it spans. Pools interleave as they grow, so a region is often in several pieces; each piece carries the name, and the shared colour says they are one pool. A name rides the ground it belongs to and follows you as you pan, so wherever you are, what you are standing on is named. A node with no pool label is not a member of anything and recedes rather than joining the palette.",
    );
