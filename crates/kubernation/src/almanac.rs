...
fn page_world(cx: &mut Ctx) {
    ...
    cx.para(
        "Nodes also belong to a NODEPOOL, and a pool spanning three zones is normal. Zone stays primary - it is the failure domain, and it is what the columns are - so such a pool reads as three regions rather than one shape, which is the truth a zone outage makes matter. Pool identity travels by COLOUR and LABEL rather than by position: View \u{25b8} Pool tints each province by its pool, names each region on the map, and lists every pool in the right column with how many nodes and zones it spans. A region in more than one piece is named once, on its largest piece - the shared colour carries the rest, the way an atlas labels an archipelago. A node with no pool label is not a member of anything and recedes rather than joining the palette.",
    );
