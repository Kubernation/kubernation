...
fn page_controls(cx: &mut Ctx) {
    ...
    cx.para(
        "The Wonders menu holds the Oracle: a bring-your-own language model you consult to EXPLAIN a scope — the whole realm, a selected workload or node, or a focused concern — and which may PROPOSE a fix (scale / restart / set-image / rollback / cordon). It is advisory: the model sees a redacted, fenced summary of what Kubernation already observed (never raw cluster dumps, never Secret values) and can NEVER change the cluster itself — a proposal is validated against the live cluster and offered as a Stage button that enters the planning turn, applied only through the usual confirmed, RBAC-checked, dry-run gate. Use Preview to review exactly what will be sent before you Consult. Configure a local model with --llm-url (default: Ollama at localhost:11434) and --llm-model (default qwen3.5:35b — must be a model you've pulled, e.g. `ollama pull qwen3.5:35b`, or pass a smaller one); an API token, if any, comes from the KUBERNATION_LLM_TOKEN env var only (never written to disk). A remote endpoint publishes off-laptop, so it is off until you Arm it; replies are model-generated — verify before acting.",
    );
}
