...
//! builders), and the attention strip.

use k8sciv_core::state::model::{
    Models, NodeHealth, PodState, WorkloadRef, build_city, build_node_detail,
};
...
pub fn draw_tooltip(world: &WorldModel, cell: (u16, u16), mouse: Vec2) {
    let mut lines: Vec<(String, Color)> = Vec::new();
    match world.region_at(cell.0, cell.1) {
        Region::City(_, c) => {
...
pub fn draw_panel(panel: &Panel, observed: &ObservedWorld, models: &Models, pl: &PanelLayout) {
    let f = pl.frame;
...
pub fn draw_attention_strip(models: &Models, concern_idx: usize) {
    ...
    if models.attention.is_empty() {
        draw_text("all quiet - no concerns", 16.0, base + 26.0, 18.0, DIM);
        return;
    }
    for (i, c) in models.attention.iter().take(3).enumerate() {
        let marker = if i == concern_idx % models.attention.len() {
            "> "
        } else {
            "  "
        };
    ...
    if models.attention.len() > 3 {
        draw_text(
            format!("+{}", models.attention.len() - 3),
            screen_width() - 50.0,
            base + 20.0,
            14.0,
            DIM,
