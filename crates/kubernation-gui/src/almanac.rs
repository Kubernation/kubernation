...
fn page_controls(cx: &mut Ctx) {
    ...
    cx.heading("Menu bar");
    cx.para(
        "The top bar holds the menus: Game (context, fit, quit), View (the map overlay — terrain health, cpu/mem pressure, replica health, or namespace territory), Orders (end of turn, discard), World (namespace filter), Help. Click a title to open it.",
    );
}
